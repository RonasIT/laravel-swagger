...
class DynamicParser implements ParserInterface
{
    ...
    {
		$docblocks = explode('*', $docblock);
		$description = Arr::first($docblocks, function ($item) {
			return strlen($item) > 3 && trim($item)[0] !== '@';
		});
		...
		return isset($matches[0]) ? $description . substr($docblock, $matches[0][1]) : '';
    }
